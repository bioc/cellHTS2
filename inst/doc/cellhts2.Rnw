...
Bioconductor repository for dynamically generated vignettes.  You can
find the complete report as a zipped PDF file in the \textit{doc}
...
%FIXME: 
% You could have a look at the 'Makefile' in the inst/doc directory
%   of the vsn package, and the way the .pdf file for the convergence2
%   vignette is 'created'. Note that there is a also a convergence2.Rnw
%   stub which makes sure that that vignette is in the index!
...
the example data set for a two-way assay in the directory called 
