...
png("cellhts2-configurationplot.png", width=650, height=650)
...
\begin{figure}[tp]
\begin{center}
\includegraphics[width=0.9\textwidth]{cellhts2-configurationplot.png}
...
%
<<callvalues, echo=FALSE, results=hide>>=
...
plot(Data(xsc), Data(y), col="blue", pch=".",
   ...
   main=expression(1/(1+e^{-lambda *(z-z[0])})))
@ 
\myincfig{cellhts2-callvalues.png}{0.5\textwidth}{A sigmoidal transformation that
...
%------------------------------------------------------------
\myincfig{cellhts2-transfplots.png}{0.95\textwidth}{Comparison between
...
library("vsn")
png("transfplots.png", width=468, height=648)
par(mfcol=c(3,2))
myPlots=function(z,...) {
