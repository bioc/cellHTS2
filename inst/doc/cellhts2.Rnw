...
maximum number of plates or replicates, respectively.  The first few
lines of an example plate list file are shown in
...
\item \Robject{varianceAdjust="byBatch"}: using the content of slot
  ...
  individual normalized intensities in each group of plates (batch)
  are divided by the per-"batch of plates" MAD values (calculated
  based on "sample" wells).  This is done separately for each
...
@ 
%
\myincfig{cellhts2-callvalues.png}{180px}{A sigmoidal transformation
...
%------------------------------------------------------------
\myincfig{cellhts2-transfplots.png}{324px}{Comparison between
