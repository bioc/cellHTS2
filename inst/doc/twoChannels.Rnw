...
For dual-channel data, we propose the following preprocessing work-flow:
\begin{enumerate} 
\item \emph{Correction of plate effects}: Plate effects in each individual channel should be corrected using \Rfunction{normalizePlates} function;

\item \emph{Channel summarization}: The per-plate corrected intensities in each channel are combined using \Rfunction{summarizeChannels};

\item \emph{Per-plate normalization of the summarized intensities} If desired, \Rfunction{normalizePlates} can be called again on the single-channel values.
\end{enumerate}

Below, we follow this procedure. Thus, to make the overall distribution of intensities in the three plates comparable, 
first we remove the plate-to-plate
variations.  This will be performed by applying plate median scaling
to each replicate and channel, followed by variance adjustment:
%
<<plateMedianChannels>>=
xn <- normalizePlates(x, scale="multiplicative", method="median", varianceAdjust="byExperiment")
@ 
%
...
This can be done using the \Rfunction{summarizeChannels} function provided in the 
\textit{cellHTS2} package. 
Furthermore, in this experiment, reporter 1 ($R_1$) monitors cell viability. Thus,
...
plate median corrected intensity values in $R_1$ channel:
%
...
This procedure of data masking and channel summarization can be carried out at once using the function \Rfunction{summarizeChannels}:
%
<<summarizeChannels>>=
...
@
%
After channel summarization, we take the $\log_2$ and re-apply plate median scaling (in this case, the plate median correction consists of subtracting each plate value by the median of values in that plate, 
since after log2 trasnsformation the data are in an additive scale), 
and readjust the variance of each replicate experiment:
%
<<redo plate correction>>=
xn1 <- normalizePlates(xn1, scale="multiplicative", log=TRUE, method="median", varianceAdjust="byExperiment") 
@
%
...
%
<<summarizeReplicates>>=
xsc <- summarizeReplicates(xn1, zscore="-", summary="mean") 
@ 
%
The resulting single $z$-score value per probe is stored in the
slot \Robject{assayData} of \Robject{xsc}. The left side of Figure~\ref{twoChannels-scores} shows the boxplots of 
...
in the working directory.
%
Finally, we will save the scored \Rclass{cellHTS} object to a file. 
%
<<savex>>=
