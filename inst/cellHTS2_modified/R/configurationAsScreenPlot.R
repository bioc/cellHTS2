...
configurationAsScreenPlot <- function(x, verbose=interactive(), posControls, negControls)
{
    ...
    if(any(aux) & verbose)
        sprintf("'%s' not found among the well annotation!\n", posControls[which(aux)])
    ...
    if(length(namePos))
    {
        ## update well colors and update well colors with pos and neg controls and remove
        ## unused well annotation:
        cols.pos <- if(length(namePos)==1) "#E41A1C" else colorRampPalette(c("red", "orange"),
    ...
    if(length(nameNeg))
    {
        cols.neg <- if(length(nameNeg)==1) "#2040FF" else colorRampPalette(c("darkblue",  "blue"),
    ...
    plotScreen(ldat, zrange=range(mtW), fill=wellCols, main=" ",
               nx=pdim(x)[["ncol"]], ny=pdim(x)[["nrow"]],
               ncol=ifelse(max(plate(x))<6L, max(plate(x)), 6L))
	
    ## put correct namea given in the conf file
