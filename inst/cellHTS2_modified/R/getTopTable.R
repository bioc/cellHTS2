...
getTopTable <- function(cellHTSlist, file="topTable.txt", verbose=interactive()){		
	...
	out <- data.frame(
			...
			wellAnno = wAnno,
			finalWellAnno = as.vector(scoresWellAnno)   ## include also the final well annotation (after the screen log file)
			)	
	## add columns with 
	if(!is.null(xraw)) {
	...
	if(state(xsc)[["annotated"]]) {
		...
		out <- cbind(out, fData(xsc)[, !(n %in% tolower(c("controlStatus", tolower(names(out)))))])
	}	
	## Export everything to the file
	out = out[order(out$score, decreasing=TRUE), ]
	out$score = round(out$score, 2)
	if(verbose) {print(sprintf("Saving 'topTable' list in file '%s'\n", file))}
