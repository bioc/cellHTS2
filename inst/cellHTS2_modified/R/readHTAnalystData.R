##
readHTAnalystData = function(filenames, path=dirname(filenames), name, nrPlates, verbose=interactive()) {
	...
	## Tempory correction here : removing the files ending with ~ (temp files under unix)
	dfiles = lapply(dfiles, function(x) {
				lastChar = substr(x,nchar(x),nchar(x))
				toBeRemoved = which(lastChar == '~')
				if(length(toBeRemoved)>0) {return(x[-toBeRemoved])}
				else {return(x)}				
			}
	)
	...
	if(verbose) {
		print(sprintf("Found data in %d x %d (%d well) format.\n", dimPlate[1], dimPlate[2], nrWell))
	}
...
readHTAnalystOneReplicate <- function(filename, nrPlates, verbose=verbose) {
	...
	if(verbose) {
		print("Reading file", basename(filename), "\n")
	}
