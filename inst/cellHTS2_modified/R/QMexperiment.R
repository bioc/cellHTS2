## The main workhorse function to produce the experiment-wide QC report. In particular, this includes
## boxplots of raw data ad normalized data as well as dot plots and density plots of the control
## distributions.
QMexperiment <- function(xn, xr, path, con, allControls, allZfac)
{
    ...
    for(ch in 1:nrCh)
    {
        ...
        for (r in 1:nrReplicate)
        {
            ...
            plotTable[count + 1, ch+1] <- hwrite(hwriteImage(sprintf("boxplot_%d_%d.png", r, ch), image.border=2),
                                                 link = sprintf("boxplot_%d_%d.pdf", r, ch), center = TRUE, br = TRUE)    
