##
QMexperiment = function(xr, xn, path, con, allControls, allZfac) {
		...
		for (r in 1:nrReplicate) {
			...
			if(ch ==1) {
				if(hasNormData & !hasLessCh)  {
					plotTable[count + 1, 1] = paste(hwrite(paste("Replicate ",r, '<br>')),hwrite("Left: raw, right: normalized", br=TRUE))
				}		
				else {
			...
			plotTable[count + 1, ch+1] = hwrite(hwriteImage(sprintf("boxplot_%d_%d.png", r, ch), image.border=2), link = sprintf("boxplot_%d_%d.pdf", r, ch), center = TRUE, br = TRUE)    
			...
			if( nrPos[ch] & nrNeg[ch] & hasNormData ) {
				...
				if (!all(is.na(unlist(yvals)))) {
					...
					plotTable[count + 1, 1] = ""
					plotTable[count + 1, ch+1] =  hwrite(hwriteImage(sprintf("Controls_%d_%d.png", r, ch), image.border=2), link = sprintf("Controls_%d_%d.pdf", r, ch),center = TRUE, br = TRUE)    
					
				} else {## if !all NA
					plotTable[count + 1, 1] = ""
					plotTable[count + 1, ch+1] = hwrite("Values for 'pos' and 'neg' controls are missing.", center = TRUE, br=TRUE)
				}## else !allNA
			} else {## if nrPos & nrNeg
				plotTable[count + 1, 1] = ""
				plotTable[count + 1, ch+1] = hwrite("No controls ('pos' and 'neg') were found and/or the 'cellHTS' object is not normalized yet.", center = TRUE, br=TRUE) 
			}## else nrPos nrNeg
