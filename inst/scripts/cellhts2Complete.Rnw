...
to change to \Rpackage{cellHTS2} package, since the former package will be discontinued. In \Rpackage{cellHTS2}, we define a \Rclass{cellHTS} object as a S4 class, which inherits from class \Rclass{NChannelSet} of \Rpackage{Biobase}~\footnote{To convert a S3 class \Rclass{cellHTS} object obtained using \Rpackage{cellHTS} package to an S4 class \Rclass{cellHTS} object suitable for \Rpackage{cellHTS2}, please see the function \Rfunction{convertOldCellHTS}.}.
...
\item[Replicates scoring and summarization] involves standardizing the values for each replicate and then summarizing replicate values 
in order to obtain a single-value per probe (for example, a robust $z$-score).

\end{description}

The steps of data normalization, replicate scoring and summarization constitute the preprocessing work-flow of the screening data, 
...
The data normalization, replicates scoring and summarization functions available in \Rpackage{cellHTS2} package 
...
The preprocessing work-flow of a typical RNAi screen using \Rpackage{cellHTS2} 
package is the following:

\begin{description}
\item[(a)] Per-plate normalization to remove plate and/or edge effects.
\item[(b)] Scoring of replicate measurements (for example, compute $z$-score values).
\item[(c)] Summarization of replicates (for example, take the median value).
\end{description}


In \Rpackage{cellHTS2} package, step \textbf{(a)} is performed using the 
function \Rfunction{normalizePlates},
step \textbf{(b)} is done using the function \Rfunction{scoreReplicates} and 
step \textbf{(c)} is done using \Rfunction{summarizeReplicates} function.
In the above preprocessing work-flow, we apply step \textbf{(b)} 
before step \textbf{(c)} so that 
the summary selected for replicate summarization has the same meaning
independently of the type of the assay.

Note that this work-flow is suitable for single-channel data. For dual-channel data, further steps are required, as explained in the 
...
\item \emph{Variance adjustment}: (optional) variance adjustment of the plate intensity 
corrected data.
\end{enumerate}
...
All of the available normalization methods are described in Appendix~\ref{sec:normalizationMethods}.
...
If \Robject{varianceAdjust="none"}, no variance adjustment is performed (default).
...
This transformation can be done by calling the function \Rfunction{scores2calls}, as shown below:
%
<<call values>>=
y <- scores2calls(xsc, z0=1.5, lambda=2)
plot(Data(xsc), Data(y), col="blue", xlab="z-scores", ylab="calls", main=expression(1/(1+e^{-lambda *(z-z[0])})))
@ 
%
...
The experiment-wide HTML report presents, for each replicate, 
...
plate. If the argument \Robject{map} of \Rfunction{writeReport} function is set to \Robject{TRUE}, this plot and the plate plots of the per-plate reports
contain tooltips (information pop-up boxes) 
dispaying the annotation information at each position within the plates.
If the scored \Rclass{cellHTS} object provided for \Rfunction{writeReport} 
is not annotated with gene identifiers, the annotation information shown by the tooltips is simply the well identifiers.
...
\item \verb=score= corresponds to the summarized score calculated for each probe (data stored in the scored and summarized object \Robject{xsc});
...
@ 
%
Before calling the category summary functions, we need to order our statistic vector according to the names of the columns of 
the category matrix.
%
<<arrange probes>>=
...
We also provide in the package a normalization method for \Rfunction{normalizePlates} 
(\Robject{method="negatives"}) that consists of scaling
...
<<Z score method, eval=FALSE>>=
 xZ <- normalizePlates(x, scale="additive", log=FALSE, method="median", varianceAdjust="byPlate") 
@ 
...
Such signal gradients can be caused by differences in temperature, incubation time or concentration, etc., in different wells across a plate.
...
In a second step, each of the obtained residual values $e_{rci}$'s of the $i$th result file are divided by their median absolute deviation ($MAD_i$) 
