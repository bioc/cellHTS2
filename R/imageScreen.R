...
imageScreen <- function (object, ar=3/5, zrange, map=FALSE, anno) {
  ...
  if (missing(zrange)) {
    ## set default values
    zrange <- range(scores(object), na.rm=TRUE)
  }

  if (!missing(anno)) {
    if(length(anno)!=length(scores(object)) & (map))
       stop("Argument 'anno' is not valid! It should be a vector with the same length as 'object@score').")
  }else{## if !missing anno
    if(object@state["annotated"]){
       if ("GeneSymbol" %in% names(geneAnno(object)))
         anno <- geneAnno(object)$GeneSymbol
       else
