...
setMethod("show", signature("ROC"),
  function(object) {
    if(any(!as.logical(c(length(object@FP), length(object@TP), length(object@posNames), length(object@negNames), length(object@assayType))))){
       cat(sprintf("Incomplete %s object. \n", class(object)))
  } else {
  cat(class(object), sprintf("object derived from the '%s' called '%s'.\n", object@assayType, object@name))
  cat(sprintf("Positive control%s: %s. \nNegative control%s: %s.\n",
              ifelse(length(object@posNames)>1, "s", ""), paste(sprintf("'%s'", object@posNames), collapse=", "), ifelse(length(object@negNames)>1, "s", ""), paste(sprintf("'%s'", object@negNames), collapse=", ")))
  }
} )
...
setMethod("plot",
  ...
  if((length(x@TP) + length(x@FP))!=0 & !all(as.logical(c(length(x@negNames), length(x@posNames), length(x@assayType))))) stop("Please complete the 'ROC' object!")
