...
getDynamicRange <- function(x, verbose=interactive(), definition, posControls, negControls) {
  ...
  ## definition to use to calculate the dynamic range (specified by the user or defined based on the data)
 ...
 ## define 'definition' based on the scale of the data: positive scale -> ratio of geometric average, otherwise, difference of arithmetic averages. 
  ...
  for (ch in 1:nrChannels) {
    ...
    yy <- getReplicatesMatrix(y, channel=ch)
...
## ---------------------------------------------------------------------------
# LPB, August 2007
# Auxiliary function to be called from 'writeReport' (similar to 'getDynamicRange' function, but performs less steps)
## ---------------------------------------------------------------------------
internalGetDynamicRange <- function(x, allControls, twoWay, namePos) {
  ...
  ##   -------  Definition for the dynamic range ---------------
  ## definition to use to calculate the dynamic range is determined based 
  ## on the scale of the data: positive scale -> ratio of geometric average, otherwise, difference of arithmetic averages. 
  ...
  for (ch in 1:nrChannels) {
    ...
    if(twoWay){
     ...
     if(length(act)) nms$"activators" = c("act", "neg") 

     if(length(inh)) nms$"inhibitors" = c("neg", "inh") 
